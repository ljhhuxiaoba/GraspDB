package org.example.cyphertransform.janusGraph.schema;
...
import org.example.cyphertransform.janusGraph.schema.JanusSchema.JanusTable;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class JanusSchema extends CypherSchema<JanusGlobalState, JanusTable> {


    public static JanusSchema createEmptyNewSchema(){
        return new JanusSchema(new ArrayList<JanusTable>(), new ArrayList<>(), new ArrayList<>(), new ArrayList<>());
    }

    //todo complete
    public JanusSchema(List<JanusTable> databaseTables, List<CypherLabelInfo> labels,
    ...
    public List<IFunctionInfo> getFunctions() {
        return Arrays.asList(JanusBuiltInFunctions.values());
    }


    public enum JanusBuiltInFunctions implements IFunctionInfo{
...
//        }
        ...
        JanusBuiltInFunctions(String name, String signature, CypherType expectedReturnType, IParamInfo... params){
            this.name = name;
    ...
    public enum JanusDataType{

    }
    public static class JanusTable extends AbstractTable<JanusTableColumn, TableIndex, JanusGlobalState> {

        //todo complete
        public JanusTable(String name, List<JanusTableColumn> columns, List<TableIndex> indexes, boolean isView) {
            super(name, columns, indexes, isView);
        }

        @Override
        public long getNrRows(JanusGlobalState globalState) {
            return 0;
        }
    }

    public static class JanusTableColumn extends AbstractTableColumn<JanusTable, JanusDataType> {
        //todo complete
        public JanusTableColumn(String name, JanusTable table, JanusDataType type) {
