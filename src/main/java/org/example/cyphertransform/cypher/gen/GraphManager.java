...
public class GraphManager {
    ...
    private static int maxNodeColor = 2;
...
//todo create relationships
        for(int i = 0; i < relationshipNum; i++){
            ...
            if(options.getDatabaseType().contains("agens"))
                sb.append(" CREATE");
            else
                sb.append(" MERGE");
...
//                    sb.append("\"").append(pair.getValue()).append("\"");
    ...
    public Object generateCreateValue(IPropertyInfo propertyInfo){
            List<Object> values = new ArrayList<>();
            for(int i = 0; i < 3; i++){
                switch (propertyInfo.getType()){
                    case NUMBER:
    ...
    private AbstractNode randomColorNode(){
        ...
        //todo agensgraph,janusgraph不支持多个标签
...
for(IPropertyInfo p :labelInfo.getProperties()) {
    if (randomly.getInteger(0,100)>10)
    result.put(p.getKey(), generateValue(p));
}
        }
